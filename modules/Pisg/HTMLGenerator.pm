...
sub create_output
{
    ...
    if ($self->{cfg}->{showbignumbers}) {
        ...
        _html("<table class=\"table table-bordered table-striped table-condensed\" width=\"$self->{cfg}->{tablewidth}\">"); # Needed for sections
        $self->_questions();
        $self->_shoutpeople();
        $self->_capspeople();
        $self->_violent();
...
HTML
    ...
    _html('<div class="navbar navbar-fixed-top navbar-inverse"><div class="navbar-inner"><div class="container">');
    _html('<a class="brand" href="#">' . $title . '</a>');
    if ($self->{cfg}->{navbarlinks}) { 
      _html('<ul class="nav">');
...
sub _headline
{
    ...
    _html( <<HTML );
   <br />
   <h3 class='text-centered main_title'>$title</h3>
HTML
}
...
sub _activetimes
{
    ...
    _html("</tr><tr class='table-bordered'>");
...
sub _activenicks
{
    ...
    my $output = "";
    $output .= "<table class=\"table table-striped table-bordered table-condensed\" border=\"0\" width=\"$self->{cfg}->{tablewidth}\"><thead>";
    $output .= "<th>&nbsp;</th>";
    ...
    for (my $i = 0; $i < $self->{cfg}->{activenicks}; $i++) {
        ...
        if ($self->{cfg}->{showlines}) {
            if ($self->{cfg}->{showlinetime}) {
                $output .= "<td nowrap=\"nowrap\">" . $self->_user_linetimes($nick,$active[0]) . "</td>";
            } else {
                $output .= "<td><span class=\"label label-inverse\">$line</span></td>";
            }
        }

        $output .= "<td>" . $self->_user_times($nick) . "</td>" if ($self->{cfg}->{showtime});
        ...
        $output .= "<td>" . sprintf("%.1f", $w/$line) . "</td>"  if ($self->{cfg}->{showwpl});
        $output .= "<td>" . sprintf("%.1f", $ch/$line) . "</td>" if ($self->{cfg}->{showcpl});
        $output .= "<td>$lastseen</td>"                          if ($self->{cfg}->{showlastseen});
        $output .= "<td>\"$randomline\"</td>"                    if ($self->{cfg}->{showrandquote});
    ...
    unless ($toshow > $nicks) {
        ...
        if ($self->{cfg}->{activenicks} <  $remain) {
            _html("<br /><b><i>" . $self->_template_text('nottop') . "</i></b>");
            _html("<table class='table table-compact runners-up table-bordered'><tr>");
            for (my $i = $self->{cfg}->{activenicks}; $i < $remain; $i++) {
                ...
                $output .= "$visiblenick <span class=\"label label-inverse pull-right\">$items</span>";
                $output .= "</span></td>";
...
sub _mostusedword
{
    ...
    if (@popular) {
        ...
        _html("<table class=\"table table-bordered table-striped table-condensed\" border=\"0\" width=\"$self->{cfg}->{tablewidth}\"><tr>");
